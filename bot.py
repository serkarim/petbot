...
async def start(message: types.Message, state: FSMContext):
    try:
        ...
        if existing_nick:
            ...
            await message.answer(f"👋 <b>С возвращением, {username}!</b>\n✅ Вы уже зарегистрированы как <b>{safe_nick}</b>", reply_markup=main_menu(user_id, is_registered=True), parse_mode="HTML")
        else:
