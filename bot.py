...
async def start(message: types.Message, state: FSMContext):
    ...
    if existing_nick:
        await message.answer(f"👋 <b>С возвращением, {username}!</b>\n\n✅ Вы уже зарегистрированы как <b>{existing_nick}</b>", reply_markup=main_menu(user_id, is_registered=True), parse_mode="HTML")
    else:
        ...
        if has_pending:
            keyboard.add(InlineKeyboardButton("📋 Статус заявки", callback_data="app_status"))
        await message.answer(f"👋 <b>Привет, {username}!</b>\n\nЧтобы вступить в клан:\n1️⃣ Подать заявку\n2️⃣ Дождаться проверки\n3️⃣ Получить ссылку\n\n<b>Готовы?</b>", reply_markup=keyboard, parse_mode="HTML")
...
async def apply_start(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        await callback.message.edit_text(
            ...
            "👤 <b>Уже в клане</b> — привяжите аккаунт к существующему нику",
...
async def reg_select_existing(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        # 🔥 ВАЖНО: Экранирование HTML-тегов
    ...
    try:
        ...
        if existing:
            ...
            await callback.message.edit_text(
                ...
                f"Вы зарегистрированы как: <b>{safe_existing}</b>",
        ...
        if update_member_tg_data(nickname, tg_username, tg_id):
            ...
            await callback.message.edit_text(
                ...
                f"Вы привязаны к: <b>{safe_nick}</b>\n"
...
async def app_status(callback: types.CallbackQuery):
    try:
        ...
        text = (
            f"📋 <b>Статус заявки</b>\n"
            f"🆔 #{user_app[0]}\n"
            f"🎮 <code>{user_app[1]}</code>\n"
            f"🕒 {user_app[5]}\n"
            f"{status_emoji} <b>{user_app[6]}</b>\n"
        )
